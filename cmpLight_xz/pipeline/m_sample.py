...
class Sample:
    ...
    def load_data_for_system(self, folder):
        ...
        # load settings
        print("[RUN] Load data for system in ", folder)
    ...
    def load_hidden_state_for_system(self, folder):
        print("[RUN] loading hidden states: {0}".format(os.path.join(self.path_to_samples, folder, "hidden_states.pkl")))
        # load settings
                ...
                if key in features:
                    if "cur_phase" in key:
                        state_after_selection[key] = \
                            self.dic_traffic_env_conf['PHASE'][self.dic_traffic_env_conf['SIMULATOR_TYPE']][value[0]]
                    else:
    ...
    def make_reward(self, folder, i):
        ...
        if i % 100 == 0:
            print("[RUN] make reward for inter {0} in folder {1}".format(i, folder))
