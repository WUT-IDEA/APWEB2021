...
class STGATAgent(AgentBase):
    def __init__(self,
        ...
        self.c_state, self.h_state = [np.zeros((self.num_agents, self.LSTM_layers[0]))], [
            np.zeros((self.num_agents, self.LSTM_layers[0]))]
    ...
    def MLP(self, layers=[128, 128], attribute='cur'):
        ...
        # all_flatten_feature) for i in range(1, self.dic_agent_conf["N_LAYER"]): locals()["dense_%d"%i] = Dense(
        # self.dic_agent_conf["D_DENSE"], activation="relu", name="dense_%d"%i)(locals()["dense_%d"%(i-1)])
        for layer_index, layer_size in enumerate(layers):
            if layer_index == 0:
                h = Dense(layer_size, activation='relu', kernel_initializer='random_normal',
                          name='Dense_embed_%d_%s' % (layer_index, attribute))(In_0)
            else:
                h = Dense(layer_size, activation='relu', kernel_initializer='random_normal',
                          name='Dense_embed_%d_%s' % (layer_index, attribute))(h)
    ...
    def LSTM_net(self, d_in, lstm_size):
        ...
        Out_x, Out_c_state, Out_h_state = LSTM(lstm_size, return_sequences=False, return_state=True)(x, initial_state=[
            In_c_state, In_h_state])
    ...
    def MultiHeadsAttModel(self, l=5, d=128, dv=16, dout=128, nv=8, attribute='cur'):
        ...
        if self.nan_code:
            ...
            nan_att = Lambda(lambda x: K.batch_dot(x[0], x[1], axes=[3, 3]) / np.sqrt(dv))(
                [nan_q_transpose, nan_k_transpose])
    ...
    def choose_action(self, count, state):
        ...
        for i in range(self.num_agents):
            ...
            for feature_name in self.dic_traffic_env_conf["LIST_STATE_FEATURE"]:
                ...
                if feature_name == "cur_phase":
                    observation.extend(
                        np.array(self.dic_traffic_env_conf['PHASE'][self.dic_traffic_env_conf['SIMULATOR_TYPE']]
        ...
        print('first_layer,first_agent,first_head, future:{0}, delayed:{1}'.format(attention[0][0][0][0][0],
                                                                                   attention[0][0][0][0][1]))
    ...
    def prepare_Xs_Y(self, memory, dic_exp_conf):
        ...
        for k in range(len(sample_slice)):
            for j in range(self.num_agents):
                q_values[j][k][_action[k][j]] = _reward[k][j] / self.dic_agent_conf["NORMAL_FACTOR"] + \
    ...
    def build_network(
        ...
        att_cur_hist = Lambda(
            lambda x: K.concatenate([K.expand_dims(x[0], axis=-1), K.expand_dims(x[1], axis=-1)], axis=-1))(
            [att_cur, att_hist])
