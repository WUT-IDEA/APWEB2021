...
class Updater:
    def __init__(self, cnt_round,
        ...
        print("[RUN] Number of agents: ", dic_traffic_env_conf['NUM_AGENTS'])
    ...
    def load_sample_with_forget(self, i):
        ...
        try:
            ...
            pass
        if i %100 == 0:
            print("[RUN] load_sample for inter {0}".format(i))
        return sample_set
    ...
    def load_sample_for_agents(self):
        ...
        print("[RUN] load_sample_for_agents time: ", time.time()-start_time)

    def sample_set_to_sample_gcn_df(self,sample_set):
        print("[RUN] make results in updater sample_set_to_sample_gcn_df ")
    ...
    def update_network(self, i):
        ...
        if self.dic_traffic_env_conf["ONE_MODEL"]:
            ...
            if self.dic_exp_conf["PRETRAIN"]:
                ...
                self.agents[i].q_network.save("model/initial",
                                "aggregate_inter_{0}.h5".format(self.agents[i].intersection_id)
                                              )
                shutil.copy("model/initial/aggregate.h5",
                            os.path.join(self.dic_path["PATH_TO_MODEL"],
                            "round_0_inter_{0}.h5".format(self.agents[i].intersection_id))
                            )
            else:
    ...
    def update_network_for_agents(self):
        if self.dic_traffic_env_conf["ONE_MODEL"]:
            ...
            print("[RUN] update_network_for_agents", self.dic_traffic_env_conf['NUM_AGENTS'])
