...
from pipeline.m_generator import Generator
from pipeline.m_sample import Sample
from pipeline.m_updater import Updater
from pipeline.m_test import testmodel
from multiprocessing import Process
import random
import pickle
import pipeline.m_test
import pandas as pd
import numpy as np
...
class Pipeline:
    ...
    def _copy_sumo_file(self, path=None):
        ...
        for file_name in self._LIST_SUMO_FILES:
            shutil.copy(os.path.join(self.dic_path["PATH_TO_DATA"], file_name), os.path.join(path, file_name))
    ...
    def __init__(self, dic_exp_conf, dic_agent_conf, dic_traffic_env_conf, dic_path):
        ...
        # test_duration
        self.test_duration = []
        sample_num = 10 if self.dic_traffic_env_conf["NUM_INTERSECTIONS"] >= 10 \
            else min(self.dic_traffic_env_conf["NUM_INTERSECTIONS"], 9)
        print("[RUN] sample_num for early stopping:", sample_num)
    ...
    def generator_wrapper(self, cnt_round, cnt_gen,
        ...
        print("[RUN] make generator")
        generator.generate()
        print("[RUN] generator_wrapper end")
        return

    def updater_wrapper(self, cnt_round,
                        dic_agent_conf, dic_exp_conf, dic_traffic_env_conf, dic_path,
                        best_round=None, bar_round=None):
        updater = Updater(
            cnt_round=cnt_round,
            dic_agent_conf=dic_agent_conf,
            dic_exp_conf=dic_exp_conf,
    ...
    def run(self, multi_process=False):
        ...
        for cnt_round in range(self.dic_exp_conf["NUM_ROUNDS"]):
            print("[RUN] round %d starts" % cnt_round)
            ...
            cs = Sample(path_to_samples=train_round, cnt_round=cnt_round,
                                 dic_traffic_env_conf=self.dic_traffic_env_conf)
            ...
            if not self.dic_exp_conf["DEBUG"]:
                for cnt_gen in range(self.dic_exp_conf["NUM_GENERATORS"]):
                    path_to_log = os.path.join(self.dic_path["PATH_TO_WORK_DIRECTORY"],
                                               "train_round",
            ...
            if multi_process:
                p = Process(target=testmodel,
                            args=(self.dic_path["PATH_TO_MODEL"], cnt_round, self.dic_exp_conf["RUN_COUNTS"],
                                  self.dic_traffic_env_conf, False))
                p.start()
                p.join()
            else:
                testmodel(self.dic_path["PATH_TO_MODEL"], cnt_round, self.dic_exp_conf["RUN_COUNTS"],
            ...
            print("[RUN] round {0} ends, total_time: {1}".format(cnt_round, time.time() - round_start_time))
