        ...
        for round in round_files:
            ...
            for inter_index in range(num_intersection):
                try:
                    ...
                    df_vehicle_inter = pd.read_csv(os.path.join(round_dir, "vehicle_inter_{0}_1.csv".format(inter_index)),
...
def summary_detail_baseline(memo):
    ...
    for traffic_file in os.listdir(records_dir):
        ...
        if os.path.getsize(os.path.join(train_dir, "inter_0.pkl")) > 0:
            ...
            total_summary.append([traffic_file,ave_duration_all, avg_pressure,flow_df.shape[0],dic_agent_conf["FIXED_TIME"],arterial_duration,side_street_duration])
        else:
